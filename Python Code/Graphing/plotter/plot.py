    ...
    while i < 2013:
        try:
            if float(countries[CC].getGDP(startYear)) != 0:
                percentChange = (float(countries[CC].getGDP(i)) - float(countries[CC].getGDP(startYear)))/float(countries[CC].getGDP(startYear))
            else:
                percentChange = 0
    ...
    while i < 2013:
        try:
            if float(countries[CC].getCumulativeASNs(startYear)) != 0:
                percentChange = (int(countries[CC].getCumulativeASNs(i)) - int(countries[CC].getCumulativeASNs(startYear)))/int(countries[CC].getCumulativeASNs(startYear))
            else:
                percentChange = 0
