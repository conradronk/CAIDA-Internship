...
def writeConfig(CC, outputPath, outputFile):
    ...
    while i < 2013:
        ...
        try:
            if (float(countries[CC].getCumulativeIPs(startYear)) != 0) and (str(int(i)-1) in countries[CC].cumulativeIPs):
            	if int(countries[CC].getCumulativeIPs(str(int(i)-1))) != 0:
	                IPPercentChange = (int(countries[CC].getCumulativeIPs(i)) - int(countries[CC].getCumulativeIPs(str(int(i)-1))))/int(countries[CC].getCumulativeIPs(str(int(i)-1)))
            else:
                IPPercentChange = 0
        ...
        config.write("\t " + str(IPPercentChange)  + "\t" + str(GDPPercentChange) + "\n")
        i += 1
    config.write("EOF\n")
